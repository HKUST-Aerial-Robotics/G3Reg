...
//#pragma omp parallel for
    for (auto &item : items) {
        ...
        pcl::PointCloud<pcl::PointXYZ>::Ptr tgt_cloud = dataloader_ptr->GetCloud(config::dataset_root, item.seq, item.tgt_idx);
        FeatureMetric metric = FrontEndEval(src_cloud, tgt_cloud, std::make_tuple(item.seq, item.src_idx, item.tgt_idx), item.pose);
...
//#pragma omp critical
        {
            ...
            total++;
            LOG(INFO) << std::fixed << std::setprecision(3) << "seq: " << item.seq << " " << total << "/" << items.size()
            << ", inlier ratio: " << metric.precision << "%, TP: " << metric.tp << " assoc: " << metric.assoc_num << " t: "
            << metric.time << "ms" << " succ(1/3): " << success1 / total << "/" << success3 / total;
        };
    }
    LOG(INFO) << "Final result: " << std::fixed << std::setprecision(3) << "inlier ratio: " << precision / total << "%, TP: " << tp / total << " assoc: " << assoc_num / total << " t: "

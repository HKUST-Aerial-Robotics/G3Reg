...
namespace pcl{
	...
	template<typename T>
	void issKeyPointExtration(boost::shared_ptr<pcl::PointCloud<T>> cloud, boost::shared_ptr<pcl::PointCloud<T>> ISS, pcl::PointIndicesPtr ISS_Idx, double resolution)
	{
...
namespace travel{
    ...
    void estimateGround(pcl::PointCloud<PointT>& cloud,
        ...
        travel_ground_seg.setParams(config::max_range, config::min_range);

...
namespace gtsam {
    ...
    enum class RegularizationMethod {
        NONE, MIN_EIG, NORMALIZED_MIN_EIG, PLANE, LINE, FROBENIUS
    };
    ...
    public:
        ...
        Gaussian2GaussianFactor(Key X, Eigen::Vector3d c1, Eigen::Vector3d c2, Eigen::Matrix3d cov1,
                                Eigen::Matrix3d cov2,
                                const SharedNoiseModel &model = nullptr,
                                RegularizationMethod regularization_method = RegularizationMethod::NONE) :
                Base(model, X) {
            m_c1 = c1;
            m_c2 = c2;
    ...
    Eigen::Matrix2d
    svdRot2d(const Eigen::Matrix<double, 2, Eigen::Dynamic> &X, const Eigen::Matrix<double, 2, Eigen::Dynamic> &Y,

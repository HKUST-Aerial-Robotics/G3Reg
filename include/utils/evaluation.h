...
public:
    ...
    std::pair<bool, bool> update(FRGresult solution, const Eigen::Matrix4d &tf_gt) {
        ...
        feature_times.push_back(feature_time);
        clique_times.push_back(clique_time);
        graph_times.push_back(graph_time);
        verify_times.push_back(verify_time);
        solver_times.push_back(tf_solver_time);
...
//                LOG(INFO) << "level: " << level;
    ...
    void saveStatistics(const std::string &directory, int seq = -1) {
        ...
        std::string feature_time_filename =
                runtime_dir + "/feature_times" + (seq == -1 ? "" : std::to_string(seq)) + ".txt";
        writeStatistics(feature_time_filename, feature_times);
        std::string clique_time_filename =
                runtime_dir + "/clique_times" + (seq == -1 ? "" : std::to_string(seq)) + ".txt";
        writeStatistics(clique_time_filename, clique_times);
        std::string graph_time_filename =
                runtime_dir + "/graph_times" + (seq == -1 ? "" : std::to_string(seq)) + ".txt";
        writeStatistics(graph_time_filename, graph_times);
        std::string verify_time_filename =
                runtime_dir + "/verify_times" + (seq == -1 ? "" : std::to_string(seq)) + ".txt";
        writeStatistics(verify_time_filename, verify_times);
        std::string solver_time_filename =
                runtime_dir + "/solver_times" + (seq == -1 ? "" : std::to_string(seq)) + ".txt";

...
public:
	...
	void LoadLiDARPoses(std::string dataset_root, int seq){
		std::string pose_file = (boost::format("%s/%02d/poses.txt") % config::dataset_root % seq).str();
		//    read kitti pose txt
		std::fstream f;
		...
		Eigen::Matrix4d Tr = GetTr(config::dataset_root, seq); // lidar to camera
		std::string line;
		int num = -1;
    ...
    void ReadSeqNum(){
        std::string dataset_dir = config::dataset_root;
        ...
        for (int seq = 0; seq < 11; ++seq) {
            ...
            int frame_num = 0;
            for (boost::filesystem::directory_iterator it(velo_dir); it != boost::filesystem::directory_iterator(); ++it) {

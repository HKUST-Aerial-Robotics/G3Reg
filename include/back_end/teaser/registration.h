...
#include "back_end/teaser/geometry.h"
...
namespace teaser {
    ...
    class RegistrationSolution {
        ...
        Eigen::Matrix<bool, Eigen::Dynamic, Eigen::Dynamic> inliers;
        double clique_time, tf_solver_time, graph_time;
    };
    ...
    public:
        ...
        struct Params {
            ...
            bool rotation_inlier_selection = true;
            /**
        ...
        void printParams(){
            ...
            if (params_.rotation_estimation_algorithm == ROTATION_ESTIMATION_ALGORITHM::GNC_TLS)
                std::cout << "rotation_estimation_algorithm: GNC_TLS" << std::endl;
            else if (params_.rotation_estimation_algorithm == ROTATION_ESTIMATION_ALGORITHM::FGR)
                std::cout << "rotation_estimation_algorithm: FGR" << std::endl;
            else
                std::cout << "rotation_estimation_algorithm: UNKNOWN" << std::endl;
            std::cout << "rotation_gnc_factor: " << params_.rotation_gnc_factor << std::endl;
            std::cout << "rotation_max_iterations: " << params_.rotation_max_iterations << std::endl;
            std::cout << "rotation_cost_threshold: " << params_.rotation_cost_threshold << std::endl;
            if (params_.rotation_tim_graph == INLIER_GRAPH_FORMULATION::CHAIN)
                std::cout << "rotation_tim_graph: CHAIN" << std::endl;
            else if (params_.rotation_tim_graph == INLIER_GRAPH_FORMULATION::COMPLETE)
                std::cout << "rotation_tim_graph: COMPLETE" << std::endl;
            else
                std::cout << "rotation_tim_graph: UNKNOWN" << std::endl;
            //std::cout << "inlier_selection_mode: " << params_.inlier_selection_mode << std::endl;
            if (params_.inlier_selection_mode == INLIER_SELECTION_MODE::PMC_EXACT)
                std::cout << "inlier_selection_mode: PMC_EXACT" << std::endl;
            else if (params_.inlier_selection_mode == INLIER_SELECTION_MODE::PMC_HEU)
                std::cout << "inlier_selection_mode: PMC_HEURISTIC" << std::endl;
            else if (params_.inlier_selection_mode == INLIER_SELECTION_MODE::KCORE_HEU)
                std::cout << "inlier_selection_mode: KCORE" << std::endl;
            else
                std::cout << "inlier_selection_mode: UNKNOWN" << std::endl;
            std::cout << "rotation_inlier_selection: " << params_.rotation_inlier_selection << std::endl;
            std::cout << "kcore_heuristic_threshold: " << params_.kcore_heuristic_threshold << std::endl;
            std::cout << "use_max_clique: " << params_.use_max_clique << std::endl;
            std::cout << "max_clique_exact_solution: " << params_.max_clique_exact_solution << std::endl;
            std::cout << "max_clique_time_limit: " << params_.max_clique_time_limit << std::endl;
        }
    ...
    protected:
        ...
        Eigen::Matrix<double, 1, Eigen::Dynamic> scale_inliers_weight_;
        ...
        Eigen::Matrix<double, 3, Eigen::Dynamic> src_tims_; // 3 x N(N-1)/2
        Eigen::Matrix<double, 3, Eigen::Dynamic> dst_tims_; // 3 x N(N-1)/2
        ...
        Eigen::Matrix<int, 2, Eigen::Dynamic> src_tims_map_; // 2 x N(N-1)/2 (src_i, src_j)
        Eigen::Matrix<int, 2, Eigen::Dynamic> dst_tims_map_; // 2 x N(N-1)/2 (dst_i, dst_j) same as src_tims_map

...
//        Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> es2(other.covariance);
    ...
    double GaussianVertex::wasserstein_distance(const GraphVertex &other) {
        Eigen::Matrix3d temp =
                covariance + other.covariance - 2 * (covariance.sqrt() * other.covariance * covariance.sqrt()).sqrt();
        return temp.trace();
    }
...
//                prob(level) = std::exp(-0.5 * (s * s) / sigma2);
    ...
    clique_solver::GraphVertex::Ptr
    create_vertex(const Eigen::Vector3d &center, const VertexInfo &vertex_info, const Eigen::Matrix3d &cov,
